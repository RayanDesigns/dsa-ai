...
interface ProgressState {
  ...
  addCompletedChallenge: (challengeId: string, moduleId: string, xp: number) => void;
...
function debouncedSave(uid: string, progress: UserProgress) {
  ...
  debounceTimer = setTimeout(() => saveProgress(uid, progress), 500);
}
...
export const useProgressStore = create<ProgressState>((set, get) => ({
  ...
  addCompletedChallenge: (challengeId, moduleId, xp) => {
    const { progress } = get();
    if (!progress) return;
